# Make sure local LLM (Ollama) is properly configured before running
...
class LLMDescriptor:
    """Interface to local LLM for generating scene descriptions"""
    ...
    def generate_description(self, scene_data: str) -> str:
        """
        Generate natural language description from scene detection data

        Args:
            scene_data: JSON formatted detection data
...
def draw_text_with_bg(img, text, pos, font_scale=1.0,
                      ...
                      thickness=2, padding=10, center_vertical=False):
    """Draw (possibly multi-line) text with background.

    - `text` can contain newline characters `\n`
    - If center_vertical=False (default): treat pos as the baseline of the last line (backward compatible).
    - If center_vertical=True: treat pos.y as the vertical center of the whole text block for centering.
    """
    x, y = pos
    ...
    # Compute size of each line
    line_sizes = [cv2.getTextSize(line, font, font_scale, thickness)[0]
                  for line in lines]
    max_w = max(w for (w, _) in line_sizes)
    line_height = max(h for (_, h) in line_sizes)

    # Line spacing
    line_gap = int(line_height * 0.5) if len(lines) > 1 else 0
    total_text_height = line_height * len(lines) + line_gap * (len(lines) - 1)

    if center_vertical:
        # Interpret y as the vertical center of the whole text block
        center_y = y
        top_y = int(center_y - total_text_height / 2)

        first_baseline_y = top_y + line_height
        top_left = (x - padding, top_y - padding)
        bottom_right = (x + max_w + padding,
                        top_y + total_text_height + padding)
    else:
        # Backward compatible behavior: interpret y as the baseline of the last line
        top_left = (x - padding, int(y - total_text_height - padding))
        bottom_right = (x + max_w + padding, int(y + padding))
        first_baseline_y = int(y - total_text_height + line_height)
    ...
    for i, line in enumerate(lines):
        line_y = first_baseline_y + i * (line_height + line_gap)
        cv2.putText(img, line, (x, line_y), font,
...
def compose_ui_frame(video_frame, workflow_manager: WorkflowManager,
    ...
    for msg in warnings:
        ...
        # Message text
        text = msg.text
        draw_text_with_bg(panel, text,
                          (bubble_x1 + 65, y_pos + bubble_h // 2),
                          font_scale=0.75,
...
def collect_scene_data(workflow_manager: WorkflowManager,
                       obstacles: List[ObstacleInfo],
                       nav_features: Optional[Dict],
                       traffic_signal: TrafficSignalState,
                       blind_detected: bool,
    ...
    """
    Collect all current scene detection data and format as JSON string
    """
    ...
    data = {
        ...
        "workflow_state": workflow_manager.workflow_state.value,
        "blind_path_detected": bool(blind_detected),
        "crossing_detected": bool(crossing_detected),
        "crossing_at_position": bool(workflow_manager.crossing_at_position),
        "obstacles": [],
        "navigation": {},
        "traffic_signal": traffic_signal.value,
        "obstacle_state": workflow_manager.obstacle_state.value,
    }

    # Add obstacle information with user-view left / center / right labels
    cx_center = frame_width / 2.0 if frame_width > 0 else 0.0

    for obs in obstacles:
        x1, y1, x2, y2 = [float(x) for x in obs.bbox]
        cx = 0.5 * (x1 + x2)

        # Normalize to roughly [-0.5, 0.5] to indicate left / right offset
        if frame_width > 0:
            rel = (cx - cx_center) / frame_width
        else:
            rel = 0.0

        # Add a small buffer to avoid jitter when near the center
        if rel < -0.08:
            side = "left"
        elif rel > 0.08:
            side = "right"
        ...
        data["obstacles"].append({
            "type": str(obs.label),
            "side": side,                               # From user perspective: left / right / center
            "center_x_norm": float(round(rel, 3)),      # Negative = left, positive = right
            "position": {
                "on_path": bool(obs.on_path),
                "in_front": bool(obs.in_front),
                "bbox": [x1, y1, x2, y2],
            },
            "distance_score": float(round(obs.distance_score, 2)),
        })

    # Add navigation features
    if nav_features:
        data["navigation"] = {
            "offset_ratio": float(round(nav_features.get("offset_ratio", 0.0), 3)),
            "orientation_deg": float(round(nav_features.get("orientation_deg", 0.0), 1)),
            "aligned": bool(nav_features.get("aligned", False)),
            "direction_hint": str(nav_features.get("direction", "unknown")),
        }
...
def main():
    parser = argparse.ArgumentParser(description="Blind Navigation System v2")
    ...
    parser.add_argument("--save-path", type=str, default="output_v2.mp4")
    parser.add_argument("--no-voice", action="store_true")
    parser.add_argument("--config", type=str, default=None)
    parser.add_argument("--llm-model", type=str, default="llama3.2",
                        help="LLM model name for scene description (default: llama3.2)")
    ...
    # Initialize LLM
    llm = LLMDescriptor(model_name=args.llm_model)
    llm_available = llm.check_connection() and llm.check_model()
    if llm_available:
        print(f"✓ LLM ready ({args.llm_model}) - Press 'X' for scene description")
    else:
        print("⚠ LLM not available. Make sure Ollama is running and model is installed.")
        print(f"  Start Ollama: ollama serve")
        print(f"  Install model: ollama pull {args.llm_model}")
    ...
    current_crossing_detected = False
    current_traffic_signal = TrafficSignalState.NO_SIGNAL
    current_nav_features = None
    ...
    try:
        while True:
            if not paused:
                ...
                # Save current detection state for LLM description
                current_blind_detected = blind_detected
                current_crossing_detected = crossing_detected
                current_traffic_signal = traffic_signal
                current_nav_features = nav_features
            ...
            if key == ord('q') or key == 27:
                ...
                if llm_available:
                    print("\n" + "=" * 60)
                    print("🤖 Generating LLM Scene Description...")
                    print("=" * 60)
                    ...
                    scene_data = collect_scene_data(
                        workflow_manager,
                        last_obstacles,
                        current_nav_features,
                        current_traffic_signal,
                        current_blind_detected,
                        current_crossing_detected,
                        width,
                        height
                    )
                    ...
                    description = llm.generate_description(scene_data)
                    ...
                    # Also speak the description if voice is enabled
                    if voice_manager.enabled:
                        voice_manager.speak(description, urgent=True)

                    prefix = "LLM: "
                    raw_text = prefix + description.strip()
